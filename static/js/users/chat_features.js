...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const pickr = Pickr.create({
    el: "#color-picker",
    theme: "classic",
    swatches: [defaultColor, "#ff0000", "#00ff00", "#0000ff"],
    components: {
      preview: true,
      opacity: true,
      hue: true,
  ...
  document.addEventListener("click", function (event) {
    ...
    const isColorPickerContainerDisplayed =
      window.getComputedStyle(colorPickerContainer).display === "flex";
    const isColorPickerContainerClicked = colorPickerContainer.contains(
      event.target
    );
    const isPickrAppClicked = pickrApp && pickrApp.contains(event.target);
    const isChangeBgColorButtonClicked = changeBgColorButton.contains(
      event.target
    );

    if (
      isColorPickerContainerDisplayed &&
  ...
  document.body.addEventListener("click", (event) => {
    if (event.target.classList.contains("file-link")) {
      ...
      const translatedText = translate(
        "This file was shared by another user. Do you trust this source and want to download: "
      );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function removeBlockedMessage() {
    const blockedMessage =
      chatMessagesContainer.querySelector(".blocked-message");
    if (blockedMessage) {

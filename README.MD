...
The GiantBomb API is temporarily unavailable.  
You must populate the database locally using the provided JSON files.

1. Ensure db_data folder with the necessary .json files is inside the 'utils' folder.
2. Run the population scripts:
- From project root run:
```
cd utils
python populate_db.py
python get_games_data.py
```
What this does:
- Creates or updates `playstyle_db.sqlite3` in the utils folder.
- Populates Games, Characters, Franchises, GameModes, Reviews
- Skips existing rows safely
- Prints how many rows were inserted per table
- Fills News and Deals data.

---

#### Rules
- Always run scripts from `utils`
- Keep JSON files in `utils/db_data`
- Do not move the database file
...
1. **Clone the Repository:** `git clone https://github.com/miron-alexandru/PlayStyleCompass.git`
...
### Option 3: Run the website using Docker
1. **Clone the Repository:** 
    ```
    git clone https://github.com/miron-alexandru/PlayStyleCompass.git
    cd PlayStyleCompass
    ```
2. **Create environment file.**
   - Copy .env.example to .env.
   - Fill the variables with your own values.
   - Set DEBUG=True for local usage.
   - Set SECURE_CONNECTION=False when running without Nginx.
3. **Apply Migrations:** 
```
python manage.py makemigrations
python manage.py migrate
...
4. **Build the Docker image.**
    - From the project root, where manage.py and Dockerfile are located: 
    ```docker build -t playstyle_app . ```
5. **Run the container**
    ```docker run --env-file .env -p 8000:8000 playstyle_app```
6. **Open the app.**
    - Without Nginx: open http://127.0.0.1:8000/en/
    - With Nginx: open https://localhost/ 
...
### Version v2.6:
* Refactored the online status system.
* Updated necessary tests.
* Refactored user profile view and template.
* Implemented/Updated dark theme where needed.
* Improved CSS on user profile, messages pages, and shared content.
* Adjusted layout and sizes for cleaner structure.
* Fixed incorrect counts for queued games and favorite games on the user profile.
* Updated text in multiple areas and refreshed translations.
* Improved CSS on game lists.
* Fixed errors when marking notifications as read or deleting them.
* Fixed a style issue where input styles affected buttons.
* Moved warnings and the update preferences button on the preferences page.
* Moved shared SCSS classes for consistent global styling.
* Updated profile dropdown labels, order, and styling.
* Fixed a JavaScript issue that blocked editing of game list reviews.
* Replaced SCSS in game_list_detail with plain CSS for simpler maintenance.
* Updated Index page with a redesigned header and news layout.
* Refined navbar structure and styling.
* Introduced SCSS for easier and cleaner style management.

### Version v2.5:
* Tested helper functions for the 'users' & 'playstyle_compass' apps.
* Completed testing for the 'users' app views.

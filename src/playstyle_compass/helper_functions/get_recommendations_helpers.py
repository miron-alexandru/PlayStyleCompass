        ...
        for matching_game in matching_title_games:
            if matching_game not in unique_games:
                ...
                for genre_game in matching_genre_games:
                    ...
                    try:
                        release_date = datetime.strptime(genre_game.release_date, '%Y-%m-%d').date()
                    except ValueError:
                        release_date = datetime.strptime(genre_game.release_date + '-01-01', '%Y-%m-%d').date()

                    if release_date >= current_date:
                        games_to_exclude.append(genre_game)

    matching_games["gaming_history"] = [game for game in matching_games["gaming_history"] if game not in games_to_exclude]
...
def apply_filters(favorite_genres, preferred_platforms, matching_games):
    ...
    matching_games["favorite_genres"] = Game.objects.filter(genre_filters).exclude(exclude_upcoming)
    matching_games["common_genres_platforms"] = Game.objects.filter(common_filters).exclude(exclude_upcoming)
    matching_games["preferred_platforms"] = Game.objects.filter(platform_filters).exclude(exclude_upcoming)
